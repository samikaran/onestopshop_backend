...
from accounts.models import User, user_directory_path, Profile
...
class Tag(models.Model):
    ...
    slug = models.SlugField("Tag slug", max_length=30,
...
class Product(models.Model):
    ...
    category = models.ForeignKey(
        Category, on_delete=models.RESTRICT, null=True, blank=True, related_name="category")
    ...
    type = models.CharField(choices=PRODUCT_TYPE,
                            max_length=100, default="regular")
    ...
    sku = ShortUUIDField(unique=True, length=5, max_length=50,
    ...
    def order_count(self):
        order_count = CartOrderItem.objects.filter(
            product=self, order__payment_status="paid").count()
        return order_count
    ...
    def frequently_bought_together(self):
        frequently_bought_together_products = Product.objects.filter(order_item__order__in=CartOrder.objects.filter(
            orderitem__product=self)).exclude(id=self.id).annotate(count=models.Count('id')).order_by('-id')[:3]
...
class Gallery(models.Model):
    ...
    image = models.FileField(
        upload_to=user_directory_path, default="gallery.jpg")
    # Is the image active?
    active = models.BooleanField(default=True)
...
class CartOrder(models.Model):
    ...
    saved = models.DecimalField(max_digits=12, decimal_places=2, default=0.00,
                                null=True, blank=True, help_text="Amount saved by customer")
    ...
    coupons = models.ManyToManyField('ecommerce.Coupon', blank=True)

    stripe_session_id = models.CharField(max_length=200, null=True, blank=True)
    oid = ShortUUIDField(length=10, max_length=25,
                         alphabet="abcdefghijklmnopqrstuvxyz")
    date = models.DateTimeField(default=timezone.now)
...
class CartOrderItem(models.Model):
    ...
    sub_total = models.DecimalField(max_digits=12, decimal_places=2,
                                    default=0.00, help_text="Total of Product price * Product Qty")
    shipping_amount = models.DecimalField(
    ...
    total = models.DecimalField(max_digits=12, decimal_places=2,
                                default=0.00, help_text="Grand Total of all amount listed above")
    ...
    delivery_couriers = models.ForeignKey(
        "ecommerce.DeliveryCouriers", on_delete=models.SET_NULL, null=True, blank=True)
    tracking_id = models.CharField(max_length=100000, null=True, blank=True)

    coupon = models.ManyToManyField("ecommerce.Coupon", blank=True)
...
class Address(models.Model):
    ...
    country = models.ForeignKey("addons.Tax", on_delete=models.SET_NULL,
                                null=True, related_name="address_country", blank=True)
...
class CancelledOrder(models.Model):
    ...
    orderitem = models.ForeignKey(
        "ecommerce.CartOrderItem", on_delete=models.SET_NULL, null=True)
...
class Coupon(models.Model):
    ...
    # redemption = models.IntegerField(default=1)
    date = models.DateTimeField(auto_now_add=True)
    active = models.BooleanField(default=True)
    ...
    # ShortUUID field
    cid = ShortUUIDField(length=10, max_length=25,
                         alphabet="abcdefghijklmnopqrstuvxyz")
