...
class FilterOrderAPIView(generics.ListAPIView):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        if filter == 'paid':
            ...
            orders = CartOrder.objects.filter(
                vendor=vendor, order_status="Pending", payment_status="paid").order_by("-id")
        elif filter == 'Fulfilled':
            orders = CartOrder.objects.filter(
                vendor=vendor, order_status="Fulfilled", payment_status="paid").order_by("-id")
        elif filter == 'Cancelled':
            orders = CartOrder.objects.filter(
                vendor=vendor, order_status="Cancelled", payment_status="paid").order_by("-id")
        else:
            orders = CartOrder.objects.filter(
...
class NotificationUnSeenListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        notifications = Notification.objects.filter(
            vendor=vendor, seen=False).order_by('seen')
...
class NotificationSeenListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        vendor = Vendor.objects.get(id=vendor_id)
        notifications = Notification.objects.filter(
            vendor=vendor, seen=True).order_by('seen')
        return notifications
...
# Vendor/Shop profile views section
